...
const [newRoom, setNewRoom] = useState({
  ...
  useEffect(() => {
    const fetchRooms = async () => {
      try {
        const res = await fetch("http://localhost:3000/api/v1/allrooms");
        ...
        // Group rooms by hospital ID
        const hospitalMap = new Map();
  
        data.rooms.forEach((room) => {
          const hospitalId = room.hospital?._id;
          const hospitalAddress = room.hospital?.address || "Unknown";
  
          if (!hospitalMap.has(hospitalId)) {
            hospitalMap.set(hospitalId, {
              id: hospitalId,
              name: `Hospital - ${hospitalAddress}`, // or fetch real names if available
              rooms: [],
            });
          }
  
          hospitalMap.get(hospitalId).rooms.push({
            ...room,
            id: room._id,
            status: room.isAvailable ? "Available" : "In Use",
            // lastCleaned: "2025-03-15", // ⚠️ Placeholder — set real value if available
            // equipment: 10, // ⚠️ Placeholder — update if your schema supports this
            // notes: "", // ⚠️ Add notes if supported
          });
        });
